...
type AppStatus = "viewed" | "shortlisted" | "accepted" | "rejected";

const statusMap: Record<AppStatus, { label: string; class: string }> = {
  viewed: {
    label: "Viewed",
...
const statusActions: {
  ...
  { value: "rejected", label: "Reject", icon: XCircle, class: "text-red-600" },
];

const ViewApplicationPage = ({ id = "3" }: { id?: string }) => {
  const data = applicationData;
  const [currentStatus, setCurrentStatus] = useState<AppStatus>(
    data.status as AppStatus,
  );
  console.log(id);
      ...
      <div className="mx-auto max-w-2xl px-4 py-10">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Badge
              variant="outline"
              className={`text-xs font-medium ${status.class}`}
            >
              {status.label}
            </Badge>
